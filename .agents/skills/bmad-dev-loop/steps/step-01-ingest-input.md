...
Examples:
- `"/bmad-dev-loop 4-1 4-2 4-3"` → `["4-1", "4-2", "4-3"]`
- `"/bmad-dev-loop 4-1"` → `["4-1"]`
- `"/bmad-dev-loop epic-4"` → `["epic-4"]` (expanded below)
...
If `{workflow.dry_run}` is `true`, append the `# DRY RUN` marker to the file, print the plan once more, and HALT with status `halted_dry_run`.
